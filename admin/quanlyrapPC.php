...
<head>
  ...
  <link rel="stylesheet" href="../admin/adminCSS/quanlyrapPC.css">
...
if ($ciList && mysqli_num_rows($ciList) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($ciList)) {
        echo "<tr>
                ...
                <td>{$row['ci_status']}</td>
                <td>
                    <button class='btn-delete-cinema'>Xóa</button>
                    <button class='btn-edit-cinema'>Sửa</button>
                </td>
              </tr>";
    }
    echo '</tbody></table>';
    ...
    if ($total_pages_cinemas > 1) {
        ...
        if ($pages_cinemas > 1) {
            echo '<a href="?pages_cinemas=' . ($pages_cinemas - 1) . '">Trang trước</a>';
        }
        for ($i = 1; $i <= $total_pages_cinemas; $i++) {
            if ($i == $pages_cinemas) {
                echo '<strong>' . $i . '</strong>';
            } else {
                echo '<a href="?pages_cinemas=' . $i . '&pages_rooms=1">' . $i . '</a>';
            }
        }
        if ($pages_cinemas < $total_pages_cinemas) {
            echo '<a href="?pages_cinemas=' . ($pages_cinemas + 1) . '">Trang sau</a>';
        }
        echo '</div>';
    }
} else {
...
if ($roomList && mysqli_num_rows($roomList) > 0) {
    ...
    if ($total_pages_rooms > 1) {
        echo '<div class="pagination">';
        if ($pages_rooms > 1) {
            echo '<a href="?pages_rooms=' . ($pages_rooms - 1) . '">Trang trước</a>';
        }
        for ($i = 1; $i <= $total_pages_rooms; $i++) {
            if ($i == $pages_rooms) {
                echo '<strong>' . $i . '</strong>';
            } else {
                echo '<a href="?pages_rooms=' . $i . '&pages_cinemas=1">' . $i . '</a>';
            }
        }
        if ($pages_rooms < $total_pages_rooms) {
            echo '<a href="?pages_rooms=' . ($pages_rooms + 1) . '">Trang sau</a>';
        }
        echo '</div>';
    }
} else {
